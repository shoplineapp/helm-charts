{{- define "workflow-library.argo_workflow_workflow.template" -}}
    ...
    templates:
      ...
      {{- if .Values.steps }}
      {{- range .Values.steps }}
      - {{- include "workflow-library.argo_workflow_step.template" . | nindent 8 }}
      {{- end }}
      {{- else }}
      {{- /* if no steps, use single step */}}
      {{- $defaultValue := merge (fromJson "{\"name\":\"entry\",\"steps\":[[{\"name\":\"step1\",\"template\":\"template\"}]]}") .Values }}
      - {{- include "workflow-library.argo_workflow_step.template" $defaultValue | nindent 8 }}
      {{- end }}

      {{- if .Values.containers }}
      {{- range .Values.containers }}
      {{ $value := list . .Release.Namespace .Values.name }}
      - {{- include "workflow-library.argo_workflow_container.template" $value | nindent 8}}
      {{- end }}
      {{- else }}
